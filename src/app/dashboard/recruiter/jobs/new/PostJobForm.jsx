...
import {Switch, Label,  Form, Fieldset, TextField, Input, TextArea, FieldError, Select, ListBox, Button, toast} from "@heroui/react";
...
export default function PostJobForm({company}) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!data.category) newErrors.category = "Job category is required";
        if (!data.type) newErrors.type = "Job type is required";
            ...
            <div className="max-w-3xl mx-auto bg-[#121214] border border-zinc-900 rounded-xl p-8 shadow-2xl">
                ...
                <Form onSubmit={handleSubmit} className="space-y-8" validationErrors={errors} validationBehavior='aria'>
                    ...
                    <Fieldset className="space-y-6 w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <Select className={selectBoxClass} name="type" isInvalid={!!errors.type}>
                                <Label className="text-zinc-400 font-medium text-sm mb-1 block">Job Type</Label>
                                <Select.Trigger className={triggerClasses}>
                                    <Select.Value />
                                    <Select.Indicator />
                                </Select.Trigger>
                                {errors.type && <span className="text-xs text-danger mt-1">{errors.type}</span>}
                                <Select.Popover className={popoverClasses}>
                                    <ListBox className="outline-none">
